const { EventHelpers, RequestServerHelpers, FileHelpers, URLHelpers } = await loadModules('/node_modules/helpers-vu_diep/core.js');
const { formatDataResponse, check, clearAllClassValidateHelpers, convertDateFormatHelpers, numberFormatHelpers } = await loadModules('/node_modules/helpers-vu_diep/coreFunctions.js');
// import { removeCommasHelpers, checkOutput } from "./common.js";
const { removeCommasHelpers, checkOutput } = await loadModules('/node_modules/helpers-vu_diep/common.js');
// alert("Hello from form.js");
...
class ModalHelpers extends RequestServerHelpers {
    ...
    */
    async startModalEdit(id, params = {},selectDefault ="") {
        try {
            this.modal = document.querySelector(this.modalSelector);
            if (!check(this.modal, this.modalSelector)) return;
            ...
            if(selectDefault){
                selectDefault.select.dataChoice[selectDefault.id].setChoiceByValue(response[selectDefault.key]);
            }
    ...
    hideModal() {
        ...
        if(this.restHidenModal) this.resetForm();
    }
...
class ValidateHelpers {
   ...
   * @param {Boolen} statusFile Trạng thái của form khi phát hiện có trường file mặc định là không có file
    ...
    validateForm(textSelect = false) {
        ...
        // Perform validation according to the design
        ...
        for (let key in dom) {
            ...
            // Check if item is an HTMLElement
            if (item instanceof HTMLElement) {
                ...
                // Handle case where item is a string (e.g., text content of a select)
                data[key] = item.trim();
            } else {
                console.error(`Unexpected type for item with key ${key}:`, item);
            }
        }
        return data;
    }
...
class SelectHelpers {
    ...
    async selectMore(select, api, key, label, params = {}, getParams = null) {
        ...
        this.event.removeSearch(select, searchEventHandler);
        this.event.search(select, searchEventHandler);
    ...
    async selectData(select, api, label, labelDefault) {
        ...
        if (res.status === 200) {
            ...
            choiceSelect.removeActiveItems();
            let dataChoice = this.processApiData(res, label, labelDefault);
            // thêm giá trị mặc định
            dataChoice.unshift({ value: "", label: labelDefault , selected: true });
...
class FormTableHelpers extends BaseFormHelpers {
    ...
    addInputChangeListeners() {
        this.domTbody.addEventListener('input', (event) => {
            const target = event.target;
            if (target.tagName.toLowerCase() === 'input') {
                const row = target.closest('tr');
                const id = row ? row.id : null;
                if (id) {
                    const dataItem = this.dataTotal.find(item => item.id == id);
                    if (dataItem) {
                        dataItem[target.name] = target.value;
