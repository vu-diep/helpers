const { formatApiUrl, check } = await loadModules("/node_modules/helpers-vu_diep/coreFunctions.js");
...
class EventHelpers {
  ...
  removeEvent(dom, eventType, callback) {
    const domElement = dom === this.scope ? this.scope : this.scope.querySelector(dom);
    if (!check(domElement, dom)) return;
  ...
  removeSearch(dom, callback) {
    this.removeEvent(dom, "search", callback);
  }
}
